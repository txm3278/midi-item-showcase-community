...
//     Adjust the created enchantment active effect, also creates an active effect on the current actor
//     to update the light config of the token.
...
//     Show a deactivation notification.
...
export async function sunBlade({ speaker, actor, token, character, item, args, scope, workflow, options }) {
  ...
  async function handleActivatePostActiveEffects(currentWorkflow, sourceItem) {
    ...
    if (!enchantements?.length) {
      ...
      ui.notifications.info(`${sourceItem._source.name} - The blade was deactivated.`);
      return;
    }

    // Adjust enchantment active effect for blade activation
    const enchantmentEffect = enchantements[0];
    const enchantmentEffectUpdates = {};
    foundry.utils.setProperty(enchantmentEffectUpdates, `flags.${MODULE_ID}.${ACTIVATED}`, 1);
    adjustProficiency(sourceItem, enchantmentEffect, enchantmentEffectUpdates);
    await enchantmentEffect.update(enchantmentEffectUpdates);
   ...
   *
   * @param {Item5e} sourceItem - The Sun Blade item.
   * @param {ActiveEffect} enchantment - The enchantment effect.
   * @param {object} enchantmentEffectUpdates - The enchantment effect updates to be applied.
   */
  async function adjustProficiency(sourceItem, enchantment, enchantmentEffecUpdates) {
    let proficencyValue;
    ...
    if (proficencyValue) {
      const changes = foundry.utils.deepClone(enchantment.changes);
      changes.push({
      ...
      enchantmentEffecUpdates.changes = changes;
    }
  }
