...
async function runWorkflows(argumentInput, config) {
  ...
  try {
    ...
    if (
      ...
      if (!workflowStates[workflowAction])
        throw `Undefined midiWorkflow name : ${workflowAction}`;
      ...
      if (macroUtil.debugLevel > 1)
        console.warn('Aborted flag on workflow is set to :', workflow.aborted);
    } else {
      const effectData = args[args.length - 1];
      if (!effectStates[workflowAction])
        throw `Undefined effectState : ${workflowAction}`;
    ...
    ui.notifications.error(
      `An unexpected error occurred in the execcution of the ${macroItem.name} ItemMacro. Please press <F12> and inspect the console errors for more information.`
    );
    console.group(
