...
export async function poisoner({
  ...
  // Only called for the 2014 rules.
    ...
    const applyPoisonActivity = scope.rolledItem.system.activities?.get(workflow.activity.id);
    ...
    if (!actor.items.find((i) => i.identifier === DEFAULT_POISONERS_KIT_IDENT)) {
      ui.notifications.warn(`${scope.macroItem.name} | You need the Poisoner's Kit to create ${DEFAULT_POTENT_POISON_NAME}`);
      return false;
    }
    ...
    if (rules === 'modern') {
      const incAbility = Object.keys(scope.macroItem.advancement.byType['AbilityScoreImprovement']?.[0]?.value.assignments)[0];
    ...
    if (potentPoisonVialItem) {
      ...
      // Set the save DC on the item data
        ...
        foundry.utils.setProperty(activity, 'save.dc.value', saveDC);
      }
    }
    ...
    if (potentPoisonVialItem) {
      ...
      potentPoisonVialItem = await actor.createEmbeddedDocuments('Item', [potentPoisonVialItemData])[0];
    }
