...
export async function poisoner({ speaker, actor, token, character, item, args, scope, workflow, options }) {
  ...
  /**
   * Returns a potent poison item data, it looks first in the world's items, then in MISC item compendium,
      ...
      effects: [
        {
          ...
          statuses: ["poisoned"],
          flags: {
            dae: { stackable: "noneNameOnly", specialDuration: game.release.generation >= 14 ? [] : ["turnEndSource"] },
          },
        },
        {
          type: "enchantment",
