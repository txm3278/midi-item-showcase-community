...
export async function wardingFlare({ speaker, actor, token, character, item, args, scope, workflow, options }) {
  ...
  if (args[0].tag === "TargetOnUse" && args[0].macroPass === "tpr.isPreAttacked.post") {
    await handleTargetOnUseIsPreAttackedPost(workflow, scope.macroItem, scope.options?.thirdPartyReactionResult, debug);
  } else if (args[0] === "on") {
    // DAE on item macro for warding flare effect
    await handleOnEffect(actor, scope.macroItem, debug);
  }
}
 ...
 */
async function handleTargetOnUseIsPreAttackedPost(workflow, sourceItem, thirdPartyReactionResult, debug) {
  if (!sourceItem.system.activities?.some((a) => a.uuid === thirdPartyReactionResult?.uuid)) {
    return;
  }
  const legacy = elwinHelpers.getRules(sourceItem) === "legacy";
  ...
  const improvedWardingFlare = sourceActor.identifiedItems.get(IMPROVED_WARDING_FLARE_ITEM_IDENT, "feat")?.first();
  ...
  if (
    (sourceActor.getRollData().classes?.cleric?.levels ?? 0) >= (improvedWardingFlare.system.prerequisites?.level ?? 99)
  ) {
    ...
    const targetUuid = workflow.targets?.first()?.document?.uuid;
    if (!targetUuid) {
      console.warn(`${DEFAULT_ITEM_NAME} | No valid target found.`, { targets: workflow.targets });
      return;
    }
