...
export async function channelDivinityRebukeTheViolent({
  ...
  async function handleTargetOnUseIsDamagedPost(
    ...
    // Damage the attacker
    const retributionActivity =
      sourceItem.system.activities?.getByType('save')?.[0];
    if (!retributionActivity) {
      if (debug) {
        console.warn(
          `${DEFAULT_ITEM_NAME} | Could not find valid the save activity for retribution.`
        );
      }
      return;
    }
    ...
    await DAE.setFlag(
      sourceActor,
      'rebukeTheViolentRetributionDmg',
      totalDamage
    );
    ...
    if (!player) {
      console.error(
        `${DEFAULT_ITEM_NAME} | Could not find player for actor ${sourceActor}`
      );
      return;
    }
