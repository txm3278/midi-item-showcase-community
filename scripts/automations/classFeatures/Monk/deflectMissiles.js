...
//       its data value should resolve to '@scale.monk.die.die'.
//
// Description:
...
export async function deflectMissiles({ speaker, actor, token, character, item, args, scope, workflow, options }) {
  ...
  if (args[0].tag === "OnUse" && args[0].macroPass === "postActiveEffects") {
    ...
    const throwBackAttackActivity = scope.rolledItem.system.activities?.find((a) => a.identifier === "throw-back-attack");
    ...
    if (!result?.throwBack) {
      ...
      throwBackItem.applyActiveEffects();
...
async function showDialog(sourceItem, nbUses, maxUses, canCatchDefault) {
  ...
  return foundry.applications.api.DialogV2.wait({
    ...
    render: (_, dialog) => {
      const canCatchEl = dialog.element.querySelector("[name=canCatch]");
      const throwBackEl = dialog.element.querySelector("[name=throwBack]");
      if (throwBackEl) {
