...
//   all attacked targets must be different. Also, the received excludeTargets parameter is modified to add the
...
export async function intoxicatedFrenzy({ speaker, actor, token, character, item, args, scope, workflow, options }) {
  ...
  if (args[0].tag === "OnUse" && args[0].macroPass === "postActiveEffects") {
    ...
    return canMakeExtraAttack(scope.options);
  }
}
 ...
 */
async function handleOnUsePostActiveEffects(workflow, sourceItem) {
 ...
 *
 * @param {object} options - The options for evaluating if an extra attack can be made.
 * @param {number} options.nbStartingAttacks - The number of attacks that has been made before the first call to this item.
 * @param {Token5e[]} options.attackedTargets - The list of tokens that have been attacked, it may contain duplicates if the same token was attacked more than once (should not be modified).
 * @param {Token5e[]} options.excludeTargets - The list targets that cannot be attacked (will be set with invalid targets for the item extra attacks).
 * @returns {boolean} True if an extra Flurry of Blows attack can be made, false otherwise.
 */
function canMakeExtraAttack(options) {
  const { nbStartingAttacks, attackedTargets } = options;
  if (nbStartingAttacks === undefined || attackedTargets === undefined) {
    return false;
  }
  ...
  // Add all attacked targets to the exclude list
  options.excludeTargets = [];
  attackedTargets.forEach((target) => options.excludeTargets.push(target));
  return true;
}
