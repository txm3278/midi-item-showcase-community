...
export async function spiritShield({
  ...
  if (args[0].tag === 'OnUse' && args[0].macroPass === 'preTargeting') {
    ...
    if (scope.rolledItem?.identifier === RAGE_ITEM_IDENT) {
      ...
      await handleRageOnUsePostActiveEffects(
        workflow,
        scope.macroItem,
        scope.rolledItem
      );
  ...
  function handleOnUsePreTargeting(currentWorkflow, sourceItem) {
    ...
    foundry.utils.setProperty(
      currentWorkflow,
      'options.workflowOptions.autoFastDamage',
      true
    );
    return true;
  }
   ...
   */
  async function handleRageOnUsePostActiveEffects(
    currentWorkflow,
    sourceItem,
    usedItem
  ) {
    ...
    const rageEffect = sourceActor.appliedEffects.find(
      (ae) => !ae.transfer && ae.origin?.startsWith(usedItem.uuid)
    );
    if (rageEffect) {
  ...
  async function handleTargetOnUseIsDamagedPost(
    ...
    if (!vengefulAncestorsItem) {
      if (debug) {
        console.warn(
          `${DEFAULT_ITEM_NAME} | Barbarian does not have the ${VENGEFUL_ANCESTORS_ITEM_IDENT} feature.`
        );
      }
      return;
    }
    if (
      ...
      const vengefulAncestorsActivity =
        vengefulAncestorsItem.system.activities?.getByType('damage')?.[0];
  ...
  async function handleOnUsePostActiveEffects(currentWorkflow, sourceActor) {
    ...
    const total = currentWorkflow.utilityRolls?.reduce(
      (acc, r) => acc + r.total,
      0
    );
  ...
  async function handleOnEffect(sourceActor, sourceToken, sourceItem) {
    ...
    const rageEffect = rage
      ? sourceActor.appliedEffects.find(
