 ...
 * @return {Promise<{haltEffectsApplication: true}|undefined>} If target has 0 HP returns an object to stop midi from applying the AE,
 *                                                    otherwise undefined.
 */
async function handleOnUsePreActiveEffects(workflow) {
 ...
 */
function handleTargetOnUseIsPreCheckSavePost(workflow, sourceItem, target, thirdPartyReactionResult, debug) {
  if (!sourceItem.system.activities?.some((a) => a.uuid === thirdPartyReactionResult?.uuid)) {
    return;
  }
  if (!workflow.saveActivity?.save) {
    if (debug) {
      console.warn(`${DEFAULT_ITEM_NAME} | Activity is not a save.`, workflow);
    }
    return;
  }
  ...
  const updates = {};
  foundry.utils.setProperty(updates, `system.activities.${workflow.saveActivity.id}.save.ability`, new Set(["cha"]));

  // TODO support saveActivity on other item???
  if (workflow.saveActivity.item.id === workflow.item.id) {
  ...
  Hooks.once(`midi-qol.RollComplete.${workflow.itemUuid}`, async (workflow2) => {
    ...
    if (debug) {
      console.warn(`${DEFAULT_ITEM_NAME} | midi-qol.RollComplete.`, { workflow });
    }
