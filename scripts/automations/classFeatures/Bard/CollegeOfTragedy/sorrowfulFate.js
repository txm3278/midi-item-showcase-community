...
export async function sorrowfulFate({
  ...
  function handleTargetOnUseIsPreCheckSavePost(
    ...
    if (!currentWorkflow.saveActivity?.save) {
      if (debug) {
        console.warn(
          `${DEFAULT_ITEM_NAME} | Activity is not a save.`,
          currentWorkflow
        );
      }
      return;
    ...
    if (currentWorkflow.saveActivity.item.id === currentWorkflow.item.id) {
      currentWorkflow.item = currentWorkflow.item.clone(updates, {
        keepId: true,
      });
      currentWorkflow.activity = currentWorkflow.item.system.activities.get(
        currentWorkflow.activity.id
      );
    }
    ...
    Hooks.once(
      ...
      async (currentWorkflow2) => {
        ...
        const bonusDamageActivity =
          sourceItem.system.activities?.getByType('damage')?.[0];
        if (!bonusDamageActivity) {
          console.warn(
            `${DEFAULT_ITEM_NAME} | Could not find valid the damage activity for ${sourceItem.name}.`
          );
          return;
        }
